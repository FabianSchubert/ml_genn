...
class InputBase(Input):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if input_frames > 1 and egp_name is None:
            raise RuntimeError("Neuron model does not " "support time-varying input")
    ...
    def create_input_model(
        self, base_model, batch_size: int, shape, replace_input: str = None
    ):
        ...
        if self.input_frames == 1:
            ...
            nm_copy.add_var(self.var_name, "scalar", 0.0, VarAccess.READ_ONLY_DUPLICATE)
            ...
            if batch_size == 1:
                ...
                nm_copy.add_egp(
                    self.egp_name,
                    "scalar*",
                    np.empty((self.input_frames,) + shape, dtype=np.float32),
                )
                ...
                nm_copy.add_egp(
                    self.egp_name,
                    "scalar*",
                    np.empty((self.input_frames, batch_size) + shape, dtype=np.float32),
                )
    ...
    def set_input(self, genn_pop, batch_size: int, shape, input):
        ...
        if input_shape_dims != shape:
            raise RuntimeError(
                f"Input shape {input.shape} does not match " f"population shape {shape}"
            )
        ...
        if self.input_frames == 1:
            ...
            if batch_size == 1:
                ...
                if len(input_batch_time_dims) != 0 and (
                    len(input_batch_time_dims) != 1 or input_batch_time_dims[0] != 1
                ):
                    raise RuntimeError(
                        f"Input shape {input.shape} does "
            ...
            # Otherwise
                ...
                # Otherwise, pad up to full batch
                    ...
                    genn_pop.vars[self.var_name].view[:] = np.pad(
                        batched_input, ((0, batch_size - input_batch_size), (0, 0))
                    )
        ...
        # Otherwise
            ...
            if batch_size == 1:
                # Check input shape either has no batch
                # dimension or it has a length of 1
                if len(input_batch_time_dims) != 1 and (
                    len(input_batch_time_dims) != 2 or input_batch_time_dims[0] != 1
                ):
                    raise RuntimeError(
                        f"Input shape {input.shape} does "
                        f"not match batch size {batch_size}"
