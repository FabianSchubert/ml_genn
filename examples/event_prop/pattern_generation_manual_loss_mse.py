...
for i in range(NUM_OUTPUT):
    for c in range(NUM_FREQ_COMP):
        ...
        y_star[:, i] += output_ampl[i, c] * np.sin(
            np.arange(TRIAL_STEPS) * freq_radians[c] + output_phase[i, c]
        )
...
loss = ManualGradient(mse_loss)

network = Network(default_params)
with network:
    # Populations
    input = Population(
        SpikeInput(max_spikes=len(in_spike_ids)), NUM_INPUT, record_spikes=True
    )
    hidden = Population(
        LeakyIntegrateFire(v_thresh=0.61, tau_mem=TAU_MEM, tau_refrac=None),
        NUM_HIDDEN,
        record_spikes=True,
    )
    output = Population(
        LeakyIntegrate(tau_mem=TAU_MEM, readout="var_trial"), NUM_OUTPUT
    )

    # Connections
    Connection(
        input, hidden, Dense(Normal(sd=2.5 / np.sqrt(NUM_INPUT))), Exponential(TAU_SYN)
    )
    Connection(
        hidden,
        hidden,
        Dense(Normal(sd=1.5 / np.sqrt(NUM_HIDDEN))),
        Exponential(TAU_SYN),
    )
    Connection(
        hidden,
        output,
        Dense(Normal(sd=2.0 / np.sqrt(NUM_HIDDEN))),
        Exponential(TAU_SYN),
    )

compiler = EventPropCompiler(
    example_timesteps=1000,
    losses=loss,
    optimiser=Adam(LR),
...
with compiled_net:
    ...
    metrics, cb_data = compiled_net.train(
        {input: in_spikes}, {output: y_star}, num_epochs=NUM_EPOCHS, callbacks=callbacks
    )
    end_time = perf_counter()
    print(f"Time = {end_time - start_time}s")

    n_show_epochs = 3
    index_epochs = np.linspace(0, NUM_EPOCHS - 1, n_show_epochs).astype(int)
    ...
    fig, axes = plt.subplots(
        ...
        sharex="col",
        sharey="row",
        figsize=(n_show_epochs * 3, 6),
    )

    for k, i in enumerate(index_epochs):
        error = []
        fac = NUM_TRIALS
        for c in range(NUM_OUTPUT):
            ...
            axes[c, k].set_title(f"Epoch {i}, Y{c}, (MSE={mse:.2f})")
            axes[c, k].plot(y)
            axes[c, k].plot(y_star[i][:, c], linestyle="--")
        axes[NUM_OUTPUT, k].scatter(
            cb_data["input_spikes"][0][i], cb_data["input_spikes"][1][i], s=1
        )
        axes[NUM_OUTPUT + 1, k].scatter(
            cb_data["hidden_spikes"][0][i], cb_data["hidden_spikes"][1][i], s=1
        )
